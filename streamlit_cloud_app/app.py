...
APP_SNAPSHOT_PATH = APP_DIR / "snapshots" / "gold_snapshot.json"
ROOT_SNAPSHOT_PATH = ROOT_DIR / "reports" / "gold_snapshot.json"
CATALOG = "workspace"
SCHEMA = "default"
TABLES = {
...
def get_databricks_config() -> dict[str, str]:
    config = dict(st.secrets.get("databricks", {}))
    required = ["server_hostname", "http_path", "token"]
    missing = [key for key in required if not str(config.get(key, "")).strip()]
    placeholders = [key for key in required if str(config.get(key, "")).startswith("YOUR_")]
    if missing or placeholders:
        details = ", ".join(sorted(set(missing + placeholders)))
        raise ValueError(f"Missing real Databricks secret values for: {details}")
...
def clear_live_cache() -> None:
    ...
    load_snapshot_data.clear()


def empty_snapshot_sections() -> dict[str, pd.DataFrame]:
    return {
        "kafkaVitalsSummary": pd.DataFrame(),
        "kafkaVitalsPatients": pd.DataFrame(),
        "kafkaEncountersSummary": pd.DataFrame(),
        "kafkaEncountersByType": pd.DataFrame(),
        "kafkaEncountersPatients": pd.DataFrame(),
    }


@st.cache_data(show_spinner=False)
def load_snapshot_data() -> tuple[dict[str, pd.DataFrame], str]:
    snapshot_path = APP_SNAPSHOT_PATH if APP_SNAPSHOT_PATH.exists() else ROOT_SNAPSHOT_PATH
    if not snapshot_path.exists():
        raise FileNotFoundError(f"Snapshot file not found: {APP_SNAPSHOT_PATH}")

    snapshot = json.loads(snapshot_path.read_text(encoding="utf-8"))
    data = {
        "patient360": pd.DataFrame(snapshot.get("patient360", [])),
        "careGaps": pd.DataFrame(snapshot.get("careGaps", [])),
        "clinicalMetrics": pd.DataFrame(snapshot.get("clinicalMetrics", [])),
        "diseaseRankings": pd.DataFrame(snapshot.get("diseaseRankings", [])),
        "demographics": pd.DataFrame(snapshot.get("demographics", [])),
        "comorbidityPairs": pd.DataFrame(snapshot.get("comorbidityPairs", [])),
        "returnGaps": pd.DataFrame(snapshot.get("returnGaps", [])),
        "audit": pd.DataFrame(snapshot.get("audit", [])),
        **empty_snapshot_sections(),
    }
    generated_at = str(snapshot.get("generatedAt") or datetime.fromtimestamp(snapshot_path.stat().st_mtime, timezone.utc))
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.caption("Databricks SQL with snapshot fallback")
    ...
    try:
        ...
        try:
            data, snapshot_generated_at = load_snapshot_data()
            mode = "Snapshot fallback"
        except Exception as snapshot_error:
            st.error("Could not load live Databricks data or the fallback snapshot.")
            st.exception(snapshot_error)
            return
    ...
    with st.sidebar:
        if mode == "Live Databricks":
            st.success("Live Databricks")
        else:
            st.warning("Snapshot fallback")
            st.caption(f"Last updated: {snapshot_generated_at}")

    if mode != "Live Databricks":
        st.warning(
            f"Snapshot fallback - last updated {snapshot_generated_at}. "
            "Live Databricks compute is unavailable, starting, or quota-limited."
        )
